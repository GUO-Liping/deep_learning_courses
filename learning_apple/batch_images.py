...
def transfer(infile, outfile):
	...
	if width == height:
		...
		set_width = 5.0  # 7.5 cm
	set_dpi = 100  # 300 pixel/inch
...
if __name__ == '__main__':
	for root, dirs, files in os.walk("apple_te\\"): ##ori_ img为需要修改的图片存储的文件夹名字
		for item in files:
			list = item. split(".")
			print(list[-1])
			if(list[-1] == "png"):
				...
				new_name = list[0] + ".jpg"
				transfer("apple_te\\" + item, "apple_test\\" + new_name )
			elif(list[-1] == "jpg"):
				new_name = list[0] + ".jpg"
				transfer("apple_te\\" + item, "apple_test\\" + new_name )
			else:
